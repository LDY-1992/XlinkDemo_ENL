...
#define Domain @"http://app.xlink.cn"
...
@interface HttpRequest_V2 ()<NSURLConnectionDataDelegate>

@property (copy, nonatomic) MyBlock myBlock;

@end

@implementation HttpRequest_V2{
...
+(void)getVerifyCodeWithPhone:(NSString *)phone didLoadData:(MyBlock)block{
    HttpRequest_V2 *req = [[HttpRequest_V2 alloc] init];
    req.myBlock = block;
...
#pragma mark 2、注册账号
+(void)registerWithAccount:(NSString *)account withPhone_zone:(NSString *)phone_zone withNickname:(NSString *)nickname withVerifyCode:(NSString *)verifyCode withPassword:(NSString *)pwd didLoadData:(MyBlock)block{
    ...
    do {
        ...
        */
        content = @{@"phone" : account, @"phone_zone" : phone_zone, @"nickname" : nickname, @"corp_id": CorpId, @"password" : pwd, @"source" : @"3"};
        HttpRequest_V2 *req = [[HttpRequest_V2 alloc] init];
        req.myBlock = block;
        NSDictionary *header = @{@"Content-Type" : @"application/json"};
...
//my_query_with_phone
+(void)QueryDataWithTableName:(NSString *)tableName withAccessToken:(NSString *)accessToken withPhone:(NSString *)uphone didLoadData:(MyBlock)block{
    do {
        HttpRequest_V2 *req = [[HttpRequest_V2 alloc] init];
        req.myBlock = block;
        NSDictionary *header = @{@"Content-Type" : @"application/json", @"Access-Token" : accessToken};
        NSString *_uphone = [[NSString alloc] initWithString:[NSString stringWithFormat:@"[%@]",uphone]];
        NSDictionary *content = @{@"query" : @{@"uphone":@{@"$in":_uphone}}};
        [req requestWithRequestType:RequestTypePOST withUrl:[NSString stringWithFormat:@"%@/v2/datas/%@", Domain, tableName] withHeader:header withContent:content];
    } while (0);
}
...
+(NSString *)getErrorInfoWithErrorCode:(NSInteger)errCode{
    ...
    switch (errCode) {
        ...
        case 4001005:errInfo = NSLocalizedString(@"注册的手机号已存在",comment: "");break;
        case 4001006:errInfo = @"注册的邮箱已存在";break;
        case 4001007:errInfo = NSLocalizedString(@"密码错误",comment: "");break;
        case 4001008:errInfo = NSLocalizedString(@"帐号不合法",comment: "");break;
        ...
        case 4041011:errInfo = NSLocalizedString(@"用户不存在",comment: "");break;
        ...
        case 5031001:errInfo = NSLocalizedString(@"服务端发生异常",comment: "");break;
            
        default:errInfo = NSLocalizedString(@"网络环境异常",comment: "");break;
    }
    return errInfo;
}
...
+(BOOL)validatePhone:(NSString *)phone{
    ...
    NSString *regex = @"^(0|86|17951)?(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])[0-9]{8}$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
